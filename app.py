...
from sim import create_new_eco, run_sim, get_dem, get_biomass
import json
import sqlalchemy
...
app = Flask(__name__) 
db = sqlalchemy.create_engine("sqlite:///enviroment.db")
...
def load_homepage():
    ...
    if steps and not steps_lg:
        ...
        STEPS, STEPS_LENGTH = int(steps), int(STEPS_LENGTH)
...
def eco_index():
    ...
    return render_template("inspector.html", eco=eco_dicts, turn=turn.turn_number)
...
def cold_open(): 
    ...
    turn.eco = eco  
    turn.Mon_Count, turn.Plant_Count, turn.Remains_Count = get_dem()
...
def continue_sim():
    for _ in range(STEPS):
        ...
        print("past break")
        eco = turn.eco 
        eco, new_turn_num = run_sim(eco, turn.turn_number, STEPS_LENGTH)
        next_turn = Turn(turn_number=new_turn_num)
        next_turn.eco = eco 
        next_turn.Mon_Count, next_turn.Plant_Count, next_turn.Remains_Count = get_dem()
        next_turn.Mon_Biomass, next_turn.Plant_Biomass, next_turn.Remains_Biomass = get_biomass() 
